...
function openStratModal(id = null) {
  _editStratId = id;
  _selColor = "#818cf8";
  ["sName", "sDesc", "sEntry", "sExit", "sRR", "sRisk", "sNotes"].forEach(
    (x) => {
  ...
  document
    .querySelectorAll("#colorPicker .cp-dot")
    .forEach((d) => d.classList.remove("sel"));
  document.querySelector("#colorPicker .cp-dot")?.classList.add("sel");
  document.getElementById("pairsCB").innerHTML = PAIRS_LIST.map(
    (p) =>
      `<label class="cb-item"><input type="checkbox" value="${p}"> ${p}</label>`,
  ).join("");
  document
    .querySelectorAll("#sessCB input")
    .forEach((cb) => (cb.checked = false));
  document.getElementById("stratModalTitle").textContent = id
    ? "Editar Estratégia"
    : "Nova Estratégia";
...
async function saveStrategy() {
  ...
  const pairs = [...document.querySelectorAll("#pairsCB input:checked")].map(
    (c) => c.value,
  );
  const sessions = [...document.querySelectorAll("#sessCB input:checked")].map(
    (c) => c.value,
  );
  const row = {
  ...
  if (_editStratId) {
    ...
    State.strategies = State.strategies.map((x) =>
      x.id === _editStratId ? mapStrategy(data) : x,
    );
  } else {
    const { data, error } = await sb
      .from("strategies")
...
async function deleteStrategy(id) {
  ...
  State.trades = State.trades.map((t) =>
    t.strategyId === id ? { ...t, strategyId: "" } : t,
  );
  refreshSelects();
  renderStrategies();
...
function renderStrategies() {
  ...
  if (!State.strategies.length) {
    el.innerHTML =
      '<div class="empty"><div class="empty-icon">⚡</div><p>Nenhuma estratégia. Cria a primeira!</p></div>';
    return;
  }
  el.innerHTML = State.strategies
    .map((s) => {
